...
def process_data_online():
    ...
    # lacc_z_list = {1:[], 2:[]}
    win_size = 128
    win_end = {1: win_size, 2: win_size}

    # Localization support variables
    leg_length = {1: 0.5, 2: 0.5}
    offset = {1: 1.0, 2: 1.0}
    first_step = {1: True, 2: True}
    no_samples = {1: 0, 2:0}
    pos_x, pos_y, acc_magnitude, pos_mag = {1: 0.0, 2: 0.0}, {1: 0.0, 2: 0.0}, {1: 0.0, 2: 0.0}, {1: 0.0, 2: 0.0}
            ...
            try:
                ...
                if no_samples[sensor_id] % 3 == 0:
                    if acc_magnitude[sensor_id] > 0.9 and np.abs(eul_z) < 100:
                        ...
                        pos_x[sensor_id] += 2 * (2 * leg_length[sensor_id]*np.cos((eul_y-10) * np.pi/180)) * np.cos((eul_x-offset[sensor_id]) * np.pi / 180) * 3.281
                        pos_y[sensor_id] += 2 * (2 * leg_length[sensor_id]*np.cos((eul_y-10) * np.pi/180)) * np.sin((eul_x-offset[sensor_id]) * np.pi / 180) * 3.281
                        pos_mag[sensor_id] += 2 * (2 * leg_length[sensor_id]*np.cos((eul_y-10) * np.pi/180)) * 3.281
                        position = str(pos_x[sensor_id])+","+str(pos_y[sensor_id])
                        try:

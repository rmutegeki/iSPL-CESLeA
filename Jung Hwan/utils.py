...
from keras.layers import LSTM, TimeDistributed, Conv1D, MaxPooling1D, Flatten, AveragePooling1D
...
def load_signals(folder, subset, signals):
    ...
    for signal in signals:
        filename = folder+'/'+subset+'/Inertial Signals/'+signal+'_'+subset+'.txt'
        signals_data.append(
...
def load_y(label_path, subset):
    ...
    """
    filename = label_path+'/'+subset+'/y_'+subset+'.txt'
    # print (filename)
    y = _read_csv(filename)[0]
    #print (y)
    #pd.get_dummies(y).as_matrix()

...
clients = [
    {"broker": "", "port": 1883, "name": "sub1", "sub_topic": "1/#", "pub_topic": "1"}
...
RAW_DATA_FILE = f"data/offline/raw_data_{dt.date()}_{dt.hour}_{dt.minute}_{dt.second}_sensor"

isStarted = False               # Change when receive the start request: True (start saving data)
currentActivity = 0            # current activity: is the index of ACTIVITIES
...
def on_message_OFFLINE(client, userdata, message):
    global currentActivity
    msg = str(message.payload.decode("utf-8"))
    print(msg)

    device_id = int(message.topic[0])
    # task_id = int(message.topic[2])  # Topic: 1 = sensor data, 2 = label
    # print("sensor_id = ", device_id, ": message = ", msg, )

    if device_id == 3:      # the device that sends label data
        currentActivity = int(msg)       # update the new activity
        print("New Activity: ", ACTIVITIES[currentActivity])
    else:
        ...
        for line in msg:
            if not line == "":
                data = f"{currentActivity}, {receive_time}, {line}"
...
def Create_connections():
    for i in range(n_clients):
        ...
        client.on_message = on_message_OFFLINE
